...
def play(file,titles,boxList,page):# Проиграть что-то
    ...
    def play_in_background_thread():# Проиграть композицию в отдельном потоке
        try:# Попробовать..
            ...
            isLocked = False
            print(page)
            renderMenu(titles,page-1,boxList) # Рендерим меню
...
def renderMenu(titles,page,boxList):
    ...
    offset=int(page*int(maxH/boxSize)-1) # Вычисляем параметр отсчёта
...
def playMusic(selectedINDEX,files,page,filesSelectIndex):
       ...
       # else: # Если это всё не наш случай, то
        ...
        boxList= []# Список "коробок" в меню(содержит его высоту)
        wait_first([play(files[filesSelectIndex],titles,boxList,page)])# Создаём генератор?
        tasks = [play(files[filesSelectIndex],titles,boxList,page)] # Создаём задачу
        task = tasks[0]
#        print(files[filesSelected])
...
while (True):# Цикл
    ...
    if(Button.DOWN in buttonsList and page+1 <= int((len(titles)/int(120/20))+1) and page+1 >0 and speaker_lock.locked()== False): # Если нажата кнопка "вниз", и мы можем переключится на следующую страницу - переключаемся
        page=page+1
        boxList= []
        renderMenu(titles,page-1,boxList) 
    elif(Button.UP in buttonsList and page >0 and speaker_lock.locked()== False): # Если мы нажали вверх, и можем идти вниз - то идём вниз.
        page=page-1
        boxList= []
        renderMenu(titles,page,boxList)
    elif(Button.CENTER in buttonsList and speaker_lock.locked()== True):
        task.close()
        #speaker_lock.release()
        #renderMenu(titles,page,boxList)
    elif(Button.LEFT in buttonsList and speaker_lock.locked()== True):
        task.close()
        filesSelectIndex =filesSelectIndex -1
        if(filesSelectIndex > len(files)-1):
            filesSelectIndex = 0
        task = playMusic(selectedINDEX,files,page,filesSelectIndex)
    elif(Button.RIGHT in buttonsList and speaker_lock.locked()== True):
        task.close()
        filesSelectIndex =filesSelectIndex +1
        if(filesSelectIndex > len(files)-1):
            filesSelectIndex = 0
        task = playMusic(selectedINDEX,files,page,filesSelectIndex)    
    elif(Button.CENTER in buttonsList and speaker_lock.locked()== False):# Играем музыку
        filesSelectIndex = 0
        if(page >1 and selectedINDEX >=1): # Если и страница и выбранный index больше 1, то
            filesSelectIndex =int((((page*4))+selectedINDEX)-3)# Индекс файла вычисляется так
        elif(page >1 and selectedINDEX <1):# Если страница больше 1, и выбранный index меньше 1 то 
   ...
   #     print("Playing!")
   #     time.sleep(0.250)
    elif(Button.LEFT in buttonsList and speaker_lock.locked()== False): # Кнопка выбора вниз
        time.sleep(0.250) # Задержка для кнопок
